import { decrypt } from "..";

describe("decrypt", () => {
  it("should be defined", () => {
    expect(decrypt).toBeDefined();
  });

  it("should decrypt strings", () => {
    const MESSAGE = `MIME-Version: 1.0
Content-Disposition: attachment; filename="smime.p7m"
Content-Type: application/x-pkcs7-mime; smime-type=enveloped-data; name="smime.p7m"
Content-Transfer-Encoding: base64


BAgXvKwUrdnMSKCABAhTeYx1COeTSAQI3Uo6hIoB70cAAAAAAAAAAAAA`;

    const KEYPAIR = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----`;

    const result = decrypt(MESSAGE, KEYPAIR);
    expect(result.toString()).toEqual("Hello world");
  });

  it("should decrypt buffers", () => {
    const MESSAGE = Buffer.from(`MIME-Version: 1.0
Content-Disposition: attachment; filename="smime.p7m"
Content-Type: application/x-pkcs7-mime; smime-type=enveloped-data; name="smime.p7m"
Content-Transfer-Encoding: base64


BAgXvKwUrdnMSKCABAhTeYx1COeTSAQI3Uo6hIoB70cAAAAAAAAAAAAA`);

    const KEYPAIR = Buffer.from(`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----`);

    const result = decrypt(MESSAGE, KEYPAIR);
    expect(Buffer.isBuffer(result)).toBe(true);
    expect(result.toString()).toEqual("Hello world");
  });
});
